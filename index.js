...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Serendia ❤️ Orchi" }, status: "idle" });
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#00ffdd").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : ""}`).setTimestamp();
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Ceza Koruma" }).catch();
};
  ...
  if (kufur.some(word => msg.content.includes(word))) {
    try {
      ...
      if (!msg.member.hasPermission("BAN_MEMBERS")) {
              msg.delete();
                return msg.channel.send(new MessageEmbed().setDescription(`${msg.author} Bu sunucuda küfür filtresi etkin.`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
      }              
    } catch(err) {
      console.log(err);
    }
...
client.on("message", async msg => {
  ...
  if (reklam.some(word => msg.content.includes(word))) {
    try {
      ...
      if (!msg.member.hasPermission("BAN_MEMBERS")) {
              msg.delete();
                return msg.channel.send(new MessageEmbed().setDescription(`${msg.author} Bu sunucuda reklam filtresi etkin.`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
      }              
    } catch(err) {
      console.log(err);
    }
...
client.on("messageUpdate", (oldMessage, newMessage) => {
  ...
  if (reklam.some(word => newMessage.content.includes(word))) {
    try {
      if (!oldMessage.member.hasPermission("BAN_MEMBERS")) {
            oldMessage.delete();
                return oldMessage.channel.send(new MessageEmbed().setDescription(`${oldMessage.author} Mesajını düzenleyerek reklam yapmak? Akıllısın :wink:`).setColor('0x800d0d').setAuthor(oldMessage.member.displayName, oldMessage.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
      }              
    } catch(err) {
      console.log(err);
    }
